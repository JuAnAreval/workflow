...
export function getFilteredVariablePickerGlobalEntriesHandler(
  ...
  return ctx.variablePickerGlobalEntries.filter((entry: VariablePickerGlobalEntry) => {
    if (
      normalizeVariablePickerSearch(
        `${entry.key} ${entry.valueTypeLabel}`,
      ).includes(search)
    ) {
      return true;
    }
    ...
    return entry.candidates.some((candidate) => {
      const sourceLabel = `${candidate.sourceTypeLabel} ${candidate.sourceNodeLabel} ${candidate.valueTypeLabel}`;
    ...
    .map((group: VariablePickerOriginGroup) => {
      ...
      const variables = groupMatches
        ...
        : group.variables.filter((variable) =>
            normalizeVariablePickerSearch(
              `${variable.key} ${variable.valueTypeLabel}`,
            ).includes(search),
          );
...
export function readValueByVariableTargetFieldValue(
  ...
  if (field === 'formFieldInstruction') {
    const index = Number(ctx.variablePickerFormFieldInstructionIndex);
    if (!Number.isInteger(index) || index < 0) {
      return '';
    }

    const fieldRow = ctx.actionFormFields?.[index];
    return typeof fieldRow?.helpText === 'string' ? fieldRow.helpText : '';
  }
...
export function writeValueByVariableTargetFieldHandler(
  ...
  if (field === 'formFieldInstruction') {
    ...
    fieldRow.helpText = String(value ?? '');
    return;
  }
