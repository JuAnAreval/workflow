...
import { composeNodeConfigToSave } from './workflow-editor.config.utils';
import {
  applyWorkflowZoom,
  centerWorkflowView,
...
} from './workflow-editor.actions.handlers';
...
} from './workflow-editor.data.handlers';
import {
...
import {
  ...
  onFormFieldInstructionInputHandler,
  onFormFieldNameInputHandler,
  onFormFieldOptionLabelInputHandler,
  onFormFieldOptionValueInputHandler,
  onFormFieldRequiredToggleHandler,
  onFormFieldTypeChangeHandler,
  ...
  removeFormFieldOptionRowHandler,
  removeHttpHeaderRowHandler,
} from './workflow-input.handlers';

export class WorkflowEditorRuntimeBase {
  protected get s(): any {
    return this as any;
  }
  ...
  onFormFieldTypeChange(index: number, event: Event): void {
    onFormFieldTypeChangeHandler(this, index, event);
  }

  onFormFieldRequiredToggle(index: number, event: Event): void {
    onFormFieldRequiredToggleHandler(this, index, event);
  }

  onFormFieldInstructionInput(index: number, event: Event): void {
    onFormFieldInstructionInputHandler(this, index, event);
  }

  openVariablePickerForFormFieldInstruction(index: number): void {
    if (!Number.isInteger(index) || index < 0) {
      return;
    }

    this.s.variablePickerFormFieldInstructionIndex = index;
    this.s.openVariablePickerForField('formFieldInstruction');
  }

  captureFormFieldInstructionVariableSelection(index: number, event: Event): void {
    if (!Number.isInteger(index) || index < 0) {
      return;
    }

    this.s.variablePickerFormFieldInstructionIndex = index;
    this.s.captureVariableFieldSelection('formFieldInstruction', event);
  }

  onFormFieldInstructionVariableFocus(index: number): void {
    if (!Number.isInteger(index) || index < 0) {
      return;
    }

    this.s.variablePickerFormFieldInstructionIndex = index;
    this.s.onVariableFieldFocus('formFieldInstruction');
  }

  onFormFieldInstructionVariableBlur(index: number): void {
    if (!Number.isInteger(index) || index < 0) {
      return;
    }

    this.s.variablePickerFormFieldInstructionIndex = index;
    this.s.onVariableFieldBlur('formFieldInstruction');
  }

  onFormFieldInstructionVariableKeydown(index: number, event: KeyboardEvent): void {
    if (!Number.isInteger(index) || index < 0) {
      return;
    }

    this.s.variablePickerFormFieldInstructionIndex = index;
    this.s.onVariableFieldKeydown('formFieldInstruction', event);
  }

  isFormFieldInstructionVariableFocused(index: number): boolean {
    return (
      this.s.variablePickerFormFieldInstructionIndex === index &&
      this.s.isVariableFieldFocused('formFieldInstruction')
    );
  }

  removeFormFieldInstructionToken(index: number, tokenIndex: number): void {
    if (!Number.isInteger(index) || index < 0) {
      return;
    }

    this.s.variablePickerFormFieldInstructionIndex = index;
    this.s.removeTokenByField('formFieldInstruction', tokenIndex);
  }

  addFormFieldOptionRow(fieldIndex: number): void {
    addFormFieldOptionRowHandler(this, fieldIndex);
  }

  removeFormFieldOptionRow(fieldIndex: number, optionIndex: number): void {
    removeFormFieldOptionRowHandler(this, fieldIndex, optionIndex);
  }

  onFormFieldOptionValueInput(
    fieldIndex: number,
    optionIndex: number,
    event: Event,
  ): void {
    onFormFieldOptionValueInputHandler(this, fieldIndex, optionIndex, event);
  }

  onFormFieldOptionLabelInput(
    fieldIndex: number,
    optionIndex: number,
    event: Event,
  ): void {
    onFormFieldOptionLabelInputHandler(this, fieldIndex, optionIndex, event);
  }
  ...
  protected composeNodeConfigToSave(showErrors = true): string | null {
    const result = composeNodeConfigToSave({
      ...
      actionFormMessageTemplate: this.s.actionFormMessageTemplate,
