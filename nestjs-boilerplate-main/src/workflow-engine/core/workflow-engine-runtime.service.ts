...
} from '../contracts/workflow-form.types';
...
type WorkflowJsonBySource = Record<
  WorkflowSavedJsonSourceType,
  WorkflowJsonByNode
>;
...
type IncomingEdgeSource = {
  sourceId: string;
  routeKey: string | null;
};

type WorkflowExecutionState = {
  incomingMap: Map<string, IncomingEdgeSource[]>;
  nodeResultById: Map<string, WorkflowExecutionNodeResult>;
};

type QueuedWorkflowNodeExecution = {
  nodeId: string;
...
type WorkflowExecutionPhase = 'run' | 'resume';

@Injectable()
export class WorkflowEngineRuntimeService {
  private readonly logger = new Logger(WorkflowEngineRuntimeService.name);
  private readonly projectCreatedTriggerType = 'trigger_project_created';
  ...
  private readonly triggerTypeByEntity: Readonly<
    Record<WorkflowEntityType, string>
  > = {
  ...
  private readonly workflowExecutionLogsEnabled =
    process.env.WORKFLOW_EXECUTION_LOGS !== 'false';
  private readonly workflowMaxExecutionSteps = this.resolvePositiveIntFromEnv(
    'WORKFLOW_MAX_EXECUTION_STEPS',
    500,
    10,
    10000,
  );
  private readonly httpRequestTimeoutMs = this.resolvePositiveIntFromEnv(
    'WORKFLOW_HTTP_TIMEOUT_MS',
    15000,
    1000,
    120000,
  );
  private readonly javascriptExecutionTimeoutMs =
    this.resolvePositiveIntFromEnv('WORKFLOW_JS_TIMEOUT_MS', 120, 10, 10000);
  private readonly supportedFormFieldTypes = new Set<WorkflowFormFieldType>([
    'text',
    'number',
    'email',
    'password',
  ...
  private readonly prevTemplateTokenRegex = /\{\{\s*prev\.([^{}]*?)\s*\}\}/gi;
  ...
  async runWebhookTrigger(
    ...
    const workflows =
      await this.workflowRepository.findByIds(matchingWorkflowIds);
    if (!workflows.length) {
      return {
        executed: false,
        workflowIds: [],
  ...
  async resumeFromCompletedFormAssignment(
    ...
    if (assignment.entityType !== 'form') {
      this.logWorkflowExecutionFinished(
        executionId,
        assignment.sourceWorkflowId ?? '(sin-workflow)',
        0,
        0,
        0,
        'resume',
    ...
    const nextNodeIds = this.collectOutgoingTargetIds(
    ...
    const queue: QueuedWorkflowNodeExecution[] = nextNodeIds.map((nodeId) => ({
      nodeId,
      context: this.cloneConditionContext(context),
    }));
    this.logWorkflowExecutionStarted(
      executionId,
      workflowId,
      context,
      'resume',
    ...
    const executionState: WorkflowExecutionState = {
      incomingMap: this.mapIncomingEdges(edges, nodeById),
      nodeResultById: new Map<string, WorkflowExecutionNodeResult>(),
    };
    ...
    while (queue.length > 0) {
      ...
      if (!nodeId || visited.has(nodeId) || !queueItem) {
        continue;
      }
      ...
      if (executedSteps >= this.workflowMaxExecutionSteps) {
        this.logger.warn(
          `workflow.max_steps executionId=${executionId} workflowId=${workflow.id} limit=${this.workflowMaxExecutionSteps} phase=resume`,
        );
        break;
      }
      ...
      let loggedOutcome = 'continueAll';
      let loggedRouteKey: string | null = null;
      try {
        ...
        loggedOutcome = outcome.kind;
        if (outcome.kind === 'route') {
          loggedRouteKey = outcome.routeKey;
        }
      } catch {
        this.logger.warn(
        ...
        executedSteps += 1;
        this.logWorkflowNodeExecution({
          executionId,
          workflowId: workflow.id,
          nodeId: node.id,
          nodeType: node.type,
          outcome: loggedOutcome,
          routeKey: loggedRouteKey,
      ...
      for (const nextNodeId of nextNodeIds) {
        if (!visited.has(nextNodeId)) {
          queue.push({
            nodeId: nextNodeId,
            context: this.cloneConditionContext(queueItem.context),
          });
        }
      }
    }
  ...
  private async executeWorkflow(
    ...
    const executionId = this.createExecutionId();
    this.logWorkflowExecutionStarted(executionId, workflow.id, context, 'run');
    ...
    const triggerNodeIds = nodes
      ...
      .filter(
        (node) => !scheduleTriggerNodeId || node.id === scheduleTriggerNodeId,
      )
      .map((node) => node.id);
    ...
    const queue: QueuedWorkflowNodeExecution[] = this.sortNodeIdsByVisualOrder(
      [...triggerNodeIds],
      nodeById,
    ).map((nodeId) => ({
      nodeId,
    ...
    while (queue.length > 0) {
      ...
      if (executedSteps >= this.workflowMaxExecutionSteps) {
        this.logger.warn(
          `workflow.max_steps executionId=${executionId} workflowId=${workflow.id} limit=${this.workflowMaxExecutionSteps} phase=run`,
        );
        break;
      }
  ...
  private async executeNode(
    ...
    if (nodeType === this.decisionConditionType) {
      this.logger.warn(
        `Nodo decision ${node.id} (${this.decisionConditionType}) no soportado. Se detiene esa rama del flujo.`,
      );
      return { kind: 'stop' };
    }
  ...
  private async executeCreateProjectAction(
    ...
    const ownerUser =
      context.actorUser ?? context.project?.user ?? context.task?.project?.user;
    if (!ownerUser) {
      this.logger.warn(
    ...
    context.workflowProject = createdProject;

    executionState.nodeResultById.set(node.id, {
  ...
  private async executeCreateTaskAction(
    ...
    const config = this.parseNodeConfig(node.config);
    const project = this.resolveWorkflowProjectForNode(
      node.id,
      context,
      executionState,
    );
    ...
    const sourceLabel = project.name?.trim()
      ? project.name
      : this.resolveContextSourceLabel(context);
    const missingVariables = new Set<string>();
    const taskName =
      this.resolveConfigStringWithPreviousJsonWithMetadata(
        config,
    ...
    if (assignmentTarget.isConfigured) {
      ...
      await this.workflowAssignmentsService.createFromWorkflowAction({
        ...
        contextData: this.buildAssignmentContextData(context, project),
      });
      return true;
    }
  ...
  private async executeCreateUserAction(
    ...
    const uniqueEmail = await this.resolveUniqueUserEmail(
      configuredEmail,
      context,
    );
    const passwordHash =
      await this.resolveWorkflowUserPasswordHash(configuredPassword);
  ...
  private async executeFormBuilderAction(
    ...
    const config = this.parseNodeConfig(node.config);
    const fieldDefinitions = this.resolveFormFieldDefinitions(config);
    ...
    const formDataDefaults =
      this.buildInitialFormDataFromDefinitions(fieldDefinitions);
    const fieldInstructionMissingVariables = new Set<string>();
    const resolvedFieldDefinitions = fieldDefinitions.map((field) => {
      const instructionTemplate = this.readOptionalTrimmedString(
        field.helpText,
      );
      ...
      const resolvedInstruction = this.resolveTemplatesInValueWithMetadata(
        instructionTemplate,
        context,
        fieldInstructionMissingVariables,
      );
      const resolvedInstructionAsString =
        typeof resolvedInstruction === 'string'
          ? resolvedInstruction
          : this.stringifyTemplateValue(resolvedInstruction);

      return {
        ...field,
        helpText: this.readOptionalTrimmedString(resolvedInstructionAsString),
      };
    });

    if (fieldInstructionMissingVariables.size > 0) {
      this.logger.warn(
        `Nodo ${node.id} (${node.type}) resolvio instrucciones de campos con variables faltantes (${Array.from(fieldInstructionMissingVariables).join(', ')}).`,
      );
    }

    const messageTemplate = this.readConfigString(config, 'messageTemplate');
    ...
    if (messageTemplate) {
      ...
      const asString =
        typeof resolved === 'string'
          ? resolved
          : this.stringifyTemplateValue(resolved);
      resolvedMessage = asString.trim() ? asString : null;

      if (missingVariables.size > 0) {
        this.logger.warn(
          `Nodo ${node.id} (${node.type}) resolvio messageTemplate con variables faltantes (${Array.from(missingVariables).join(', ')}).`,
        );
      }
    }

    const workflowProject = this.resolveWorkflowProjectForNode(
      node.id,
      context,
      executionState,
    );
    ...
    const assignedUser =
      ...
      this.resolveDefaultAssigneeForForm(context, workflowProject);
    if (!assignedUser) {
      this.logger.warn(
    ...
    const assignment =
      await this.workflowAssignmentsService.createFromWorkflowAction({
        ...
        sourceNodeId: node.id,
        templateData: {
          fields: resolvedFieldDefinitions.map((field) => ({
            key: field.key,
            name: field.name,
            label: field.label,
            type: field.type,
          ...
          data: formDataDefaults,
          ...(messageTemplate ? { messageTemplate } : {}),
          ...(resolvedMessage ? { message: resolvedMessage } : {}),
        },
        formData: formDataDefaults,
        contextData: {
          ...this.buildAssignmentContextData(context, workflowProject),
          formNodeId: node.id,
  ...
  private async executeJavascriptCodeAction(
    ...
    for (const row of inputRows) {
      ...
      inputValues[inputName] =
        this.parseJavascriptRuntimeValue(resolvedInputValue);
    }
    ...
    const javascriptCode = typeof rawCode === 'string' ? rawCode.trim() : '';
    if (!javascriptCode) {
      this.logger.warn(
    ...
    this.logger.log(`Nodo JS ${node.id} ejecutado en workflow ${workflowId}.`);
    return true;
  }
  ...
  private isNodeTriggeredByEvent(
    ...
    if (
      entityType === 'project' &&
      node.type === this.projectCreatedTriggerType
    ) {
  ...
  private readStructuredDecisionRules(
    value: unknown,
  ...
  private resolveOperandPathFallback(
    ...
    if (!normalized || !this.looksLikePath(normalized)) {
      return this.normalizeDecisionLiteralValue(value);
    }
    ...
    return this.normalizeDecisionLiteralValue(resolved);
  }

  private normalizeDecisionLiteralValue(value: unknown): unknown {
    if (typeof value !== 'string') {
      return value;
    }
  ...
  private evaluateDecisionRule(
    ...
    if (
      !field ||
      !operator ||
      (!isBooleanOperator && !compareWithField && !hasValue)
    ) {
      return null;
    }
  ...
  private resolvePathValue(path: string, context: ConditionContext): unknown {
    ...
    if (lowerPath.startsWith('response.')) {
      ...
      return this.resolvePreviousJsonPathValue(
        previousJsonRecord,
        responsePath,
      );
    }
      ...
      if (
        !existingTargets.some(
          (entry) =>
            ...
            entry.routeKey === normalizedRouteKey,
        )
      ) {
  ...
  private mapIncomingEdges(
    edges: WorkflowEdge[],
    nodeById: Map<string, WorkflowNode>,
  ): Map<string, IncomingEdgeSource[]> {
    const incoming = new Map<string, IncomingEdgeSource[]>();
    ...
    for (const edge of edges) {
      ...
      const existingSources = incoming.get(targetId) ?? [];
      const normalizedRouteKey =
        typeof edge.routeKey === 'string' && edge.routeKey.trim()
          ? edge.routeKey.trim()
          : null;
      ...
      incoming.set(targetId, existingSources);
    }
  ...
  private cloneConditionContext(context: ConditionContext): ConditionContext {
    return {
      ...context,
    };
  }

  private resolveWorkflowProjectForNode(
    nodeId: string,
    context: ConditionContext,
    executionState: WorkflowExecutionState,
  ): Project | null {
    if (context.workflowProject) {
      return context.workflowProject;
    }

    const visited = new Set<string>();
    const queue = (executionState.incomingMap.get(nodeId) ?? []).map(
    ...
    while (queue.length > 0) {
      ...
      const result = executionState.nodeResultById.get(currentNodeId);
      if (result?.project) {
        return result.project;
      }

      const parentSources = executionState.incomingMap.get(currentNodeId) ?? [];
      for (const parentSource of parentSources) {
        if (!visited.has(parentSource.sourceId)) {
          queue.push(parentSource.sourceId);
        }
      }
    }

    return (
      context.workflowProject ??
      context.project ??
  ...
  private normalizeNodePosition(value: unknown, fallback: number): number {
    if (typeof value === 'number' && Number.isFinite(value)) {
      return value;
    }
  ...
  private resolvePreviousJsonVariables(
    ...
    const previousJsonRecord = this.resolvePreviousJsonRecord(context);
    return this.resolvePreviousJsonVariablesWithRecord(
      value,
      previousJsonRecord,
    );
  }
  ...
  private resolveRecordSegmentKey(
    ...
    if (segmentCompactId) {
      const compactIdMatches = entries.filter((candidate) => {
        ...
        return !!candidateCompactId && candidateCompactId === segmentCompactId;
      });
  ...
  private async resolveAssignmentTarget(config: NodeConfig): Promise<{
    ...
    if (assignedUserId === null) {
      return {
        isConfigured:
          rawAssignedUserId !== undefined && rawAssignedUserId !== null,
        user: null,
      };
    }
  ...
  private buildAssignmentContextData(
    context: ConditionContext,
    workflowProject?: Project | null,
  ): NodeConfig {
    const resolvedWorkflowProject =
      workflowProject ??
      context.workflowProject ??
      context.project ??
      context.task?.project;
    ...
    return {
      ...
      projectId: resolvedWorkflowProject?.id ?? null,
      workflowProjectId: resolvedWorkflowProject?.id ?? null,
  ...
  private resolveFormFieldDefinitions(
    config: NodeConfig,
  ): WorkflowFormFieldDefinition[] {
    ...
    if (Array.isArray(rawFields)) {
      for (const rawField of rawFields) {
        const parsedField = this.parseFormFieldDefinition(rawField);
        if (!parsedField) {
          continue;
        }

        const normalizedKey = parsedField.key.toLowerCase();
        if (usedKeys.has(normalizedKey)) {
          continue;
        }
        usedKeys.add(normalizedKey);
        definitions.push(parsedField);
      }
    }
    ...
    const rawData = this.asRecord(config['data']);
  ...
  private parseFormFieldDefinition(
    rawField: unknown,
    ...
    if (typeof rawField === 'string') {
      ...
      return this.createDefaultFormFieldDefinition(key);
    }
    ...
    const required =
      type === 'instruction'
        ...
        : typeof rawField['required'] === 'boolean'
          ? rawField['required']
          : true;
    const placeholder = this.readOptionalTrimmedString(rawField['placeholder']);
    const helpText = this.readOptionalTrimmedString(rawField['helpText']);
    const defaultValue = this.readOptionalTrimmedString(
      rawField['defaultValue'],
    );
    const options =
      type === 'select'
        ? this.resolveFormFieldOptions(rawField['options'])
        : [];
  ...
  private buildInitialFormDataFromDefinitions(
  ...
  private resolveFormFieldType(value: unknown): WorkflowFormFieldType {
    const normalized = String(value ?? '')
      .trim()
      .toLowerCase();
    if (this.supportedFormFieldTypes.has(normalized as WorkflowFormFieldType)) {
  ...
  private resolveFormFieldOptions(value: unknown): WorkflowFormFieldOption[] {
    ...
    for (const rawOption of value) {
      ...
      const optionValue = typeof rawValue === 'string' ? rawValue.trim() : '';
      if (!optionValue) {
        continue;
      }
  ...
  private readOptionalTrimmedString(value: unknown): string | null {
    if (typeof value !== 'string') {
      return null;
    }
  ...
  private resolveDefaultAssigneeForForm(
    context: ConditionContext,
    workflowProject?: Project | null,
  ): User | null {
    return (
      context.actorUser ??
      context.user ??
      workflowProject?.user ??
      context.workflowProject?.user ??
  ...
  private async buildContextFromFormAssignment(
    ...
    const entityType = this.resolveContextEntityType(
      contextData['triggerEntityType'],
    );
    ...
    const workflowProjectId = this.normalizeEntityId(
      contextData['workflowProjectId'] ?? contextData['projectId'],
    );
    ...
    const [
      projectFromContext,
      workflowProjectFromContext,
      taskFromContext,
      userFromContext,
    ] = await Promise.all([
      sourceProjectId
        ? this.projectRepository.findById(sourceProjectId)
        : Promise.resolve(null),
      workflowProjectId
        ? this.projectRepository.findById(workflowProjectId)
        : Promise.resolve(null),
      sourceTaskId
        ? this.taskRepository.findById(sourceTaskId)
        : Promise.resolve(null),
      sourceUserId
        ? this.userRepository.findById(sourceUserId)
        : Promise.resolve(null),
    ]);

    const webhookToken = this.readConfigString(contextData, 'webhookToken');
    const webhookMethod =
      this.readConfigString(contextData, 'webhookMethod')?.toUpperCase() ??
      'POST';
    ...
    const scheduleMode =
      this.readConfigString(contextData, 'scheduleMode') === 'once'
        ...
        : this.readConfigString(contextData, 'scheduleMode') === 'recurring'
          ? 'recurring'
          : null;
    const scheduleTimezone = this.readConfigString(
      contextData,
      'scheduleTimezone',
    );
  ...
  private resolvePositiveIntFromEnv(
    envName: string,
    fallback: number,
    min: number,
    max: number,
  ...
  private logWorkflowExecutionStarted(
    executionId: string,
    workflowId: string,
    context: ConditionContext,
    phase: WorkflowExecutionPhase,
  ): void {
    if (!this.workflowExecutionLogsEnabled) {
      return;
    }
  ...
  private logWorkflowNodeExecution(input: {
    executionId: string;
    workflowId: string;
    nodeId: string;
    nodeType: string;
  ...
  private logWorkflowExecutionFinished(
    executionId: string,
    workflowId: string,
    pausedFormAssignments: number,
    ...
    this.logger.log(
      JSON.stringify({
        event: 'workflow.execution.finish',
        executionId,
        workflowId,
        phase,
        pausedFormAssignments,
